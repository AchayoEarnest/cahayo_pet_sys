...
export default function ReportsPage() {
  ...
  function handleExport(format: "csv" | "xlsx") {
    ...
    if (activeTab === "daily" && daily) {
      ...
      rows = (Array.isArray(perf) ? perf : (perf.shifts ?? [])).map(
        (s: any) => ({
      ...
      rows = (Array.isArray(fuel) ? fuel : (fuel.variance ?? [])).map(
        (v: any) => ({
          Date: v.date ?? date,
      ...
      rows = (
        Array.isArray(attendants) ? attendants : (attendants.attendants ?? [])
      ).map((a: any) => ({
        Attendant:
          `${a.attendant__first_name ?? ""} ${a.attendant__last_name ?? ""}`.trim(),
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Operational insights and financial summaries
          </p>
        </div>
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              {
                label: "Total Revenue",
                value: formatCurrency(daily.shifts.total_revenue ?? 0),
              },
              {
                label: "Net Revenue",
                value: formatCurrency(daily.net_revenue ?? 0),
              },
              {
                label: "Litres Sold",
                value: formatLitres(daily.shifts.total_litres ?? 0),
              },
              {
                label: "Transactions",
                value: formatNumber(daily.transactions.count ?? 0),
              },
            ].map((k) => (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="card p-5">
              ...
              <div className="space-y-3">
                {daily.attendant_performance.map((a: any) => (
                  <div
                    key={a.attendant__id}
                  ...
                  >
                    <div className="w-8 h-8 bg-brand-100 rounded-full flex items-center justify-center text-brand-700 text-xs font-bold flex-shrink-0">
                      {a.attendant__first_name?.[0] ?? "?"}
                      {a.attendant__last_name?.[0] ?? ""}
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-500">
                        {a.shifts} shift(s) · {formatLitres(a.litres)}
                      </p>
                    </div>
        ...
        <div className="space-y-5">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              {
                label: "Total Revenue",
                value: formatCurrency(perf.totals.total_revenue ?? 0),
              },
              {
                label: "Total Litres",
                value: formatLitres(perf.totals.total_litres ?? 0),
              },
              {
                label: "Total Variance",
                value: formatCurrency(perf.totals.total_variance ?? 0),
              },
              {
                label: "Flagged Shifts",
                value: String(perf.totals.flagged_shifts ?? 0),
                alert: (perf.totals.flagged_shifts ?? 0) > 0,
              },
            ].map((k) => (
              <div
                key={k.label}
                className={`kpi-card ${k.alert ? "border-amber-200" : ""}`}
              >
                <p
                  className={`text-2xl font-bold ${k.alert ? "text-amber-600" : "text-gray-900"}`}
                >
                  {k.value}
                </p>
                <p className="text-xs text-gray-500">{k.label}</p>
            ...
            <div key={tank.tank_id} className="card p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="text-right">
                  <p className="text-2xl font-bold text-gray-900">
                    {(tank.fill_percentage ?? 0).toFixed(1)}%
                  </p>
              ...
              <div className="h-3 bg-gray-100 rounded-full overflow-hidden mb-4">
                <div
                  className={`h-full rounded-full transition-all ${
                    tank.fill_percentage > 50
                      ? "bg-emerald-500"
                      : tank.fill_percentage > 25
                        ? "bg-amber-500"
                        : "bg-red-500"
                  }`}
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                ...
                <div>
                  ...
                  <p className="font-medium">
                    {formatLitres(tank.litres_sold_today)}
                  </p>
                </div>
                {tank.book_variance !== null && (
                  <div>
                    ...
                    <p
                      className={`font-medium ${tank.book_variance < 0 ? "text-red-600" : "text-emerald-600"}`}
                    >
                      {tank.book_variance > 0 ? "+" : ""}
                      {formatLitres(tank.book_variance)}
                    </p>
                  </div>
                )}
              </div>
              ...
              <tbody className="divide-y divide-gray-50">
                {attendants.attendants.map((a: any) => (
                  <tr key={a.attendant__id} className="hover:bg-gray-50">
                    ...
                    <td className="table-cell">
                      {formatLitres(a.total_litres)}
                    </td>
                    <td className="table-cell">
                      {formatCurrency(a.avg_revenue_per_shift)}
                    </td>
                    <td
                      className={`table-cell font-medium ${a.total_variance < 0 ? "text-red-600" : "text-emerald-600"}`}
                    >
                      {a.total_variance >= 0 ? "+" : ""}
                      {formatCurrency(a.total_variance)}
                    </td>
                    <td className="table-cell">
                      {a.flagged_shifts > 0 ? (
